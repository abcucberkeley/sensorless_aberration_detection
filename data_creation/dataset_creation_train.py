...
def crop_center(patch_list, patch_size=128):
    cropped_patches = []
...
def check_occupancy(patch, occ_thresh=0.02, patch_size=128):
    z, y, x = patch.shape
    z = z // 2
    y = y // 2
    x = x // 2
...
def process_patch(dataset_name, res_grp, file_idx, label_patch_images, label_patch_metadata, label, seed, rot_num, rotation_mode, sample_patch_size, occupancy_thresh, num_psfs_per_bin, psf_path_list, mean_background_offset, sigma_background_noise, root, ph, num_comp_labels=1):
    gt_path = os.path.join(root, "training_128/mito_training_set_1000/", dataset_name, res_grp, "gt/")
    os.makedirs(gt_path, exist_ok=True)

    # den_path = os.path.join(root, "training_million/mito_training_set_denoise/", dataset_name, res_grp, "convolved/")
    # os.makedirs(den_path, exist_ok=True)

    input_path = os.path.join(root, "training_128/mito_training_set_1000/", dataset_name, res_grp, "input/")
    ...
    for gt_rot_count in range(rot_num, rot_num + 1):
        ...
        if check_occupancy(final_patch, occupancy_thresh, sample_patch_size):
            ...
            # io.imsave(os.path.join(gt_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + ".tif"), photons2counts(cropped_gt_patch).astype('float32'))
            io.imsave(os.path.join(gt_path, info_dict['label'] + '-' + str(file_idx) + '-rot_' + str(gt_rot_count) + '-ph_' + str(ph) + ".tif"), photons2counts(cropped_gt_patch).astype('float32'))
            # io.imsave(os.path.join(gt_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + "_" + str(factor) + ".tif"), cropped_gt_patch.astype('float32'))

            # den_patch_path = os.path.join(den_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph))
            # os.makedirs(den_patch_path, exist_ok=True)
            #
            input_patch_path = os.path.join(input_path, info_dict['label'] + '-' + str(file_idx) + '-rot_' + str(gt_rot_count) + '-ph_' + str(ph))
            ...
            for psf_path in psf_path_list:
                ...
                for psf_file_num in psf_file_nums:
                    ...
                    # io.imsave(os.path.join(den_patch_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".tif"), photons2counts(cropped_convolved_patch).astype('float32'))
                    # io.imsave(os.path.join(gt_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".tif"), photons2counts(cropped_convolved_patch).astype('float32'))

                    io.imsave(os.path.join(input_patch_path, info_dict['label'] + '-' + str(file_idx) + '-rot_' + str(gt_rot_count) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".tif"), convolved_noisy_patch.astype('float32'))

                    write_json(os.path.join(input_patch_path, info_dict['label'] + '-' + str(file_idx) + '-rot_' + str(gt_rot_count) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".json"), info_dict)

                    # io.imsave(os.path.join(input_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".tif"), convolved_noisy_patch.astype('float32'))
                    #
                    # write_json(os.path.join(input_path, info_dict['label'] + '-' + str(file_idx) + '-ph_' + str(ph) + '-psf_' + str(psf_count) + ".json"), info_dict)
...
def parse_args(args):
    ...
    parser.add_argument("--file_idx", type=int, default=1, help="File index of the augmentation patch")

    parser.add_argument("--seed", type=int, default=10000, help="Random seed")

    parser.add_argument("--sample_patch_size", type=int, default=128, help="Sample patch size")

    parser.add_argument("--occupancy_thresh", type=float, default=0.01, help="Fraction of non-zero voxels")

    parser.add_argument("--vary_intensity", type=int, default=1, help="Determine whether to vary Intensity")

    parser.add_argument("--ph", type=int, default=1000, help="Photon Factor")

    # parser.add_argument("--num_rotations", type=int, default=1, help="Number of rotations for augmentation")
    parser.add_argument("--rot_num", type=int, default=1, help="Rotation Number for augmentation")

    parser.add_argument("--rotation_mode", type=str, default="same", help="Rotation mode for determining direction for multiple labels")

    parser.add_argument("--num_psfs_per_bin", type=int, default=1, help="Number of PSFs per bin to be used for augmentation")

    parser.add_argument("--mean_background_offset", type=float, default=100, help="Mean background offset")

    parser.add_argument("--sigma_background_noise", type=float, default=40, help="Standard deviation of the noise")
...
# def main():
    ...
    rot_num = args.rot_num
    rotation_mode = args.rotation_mode
    num_psfs_per_bin = args.num_psfs_per_bin
    mean_background_offset = args.mean_background_offset
    sigma_background_noise = args.sigma_background_noise

    np.random.seed(seed + ph + file_idx + rot_num)

    root = r"/clusterfs/nvme/sayan/AI/"

    patch_dataset_root = os.path.join(root, "FIB-SEM_data_patches_375/", dataset_name, res_grp)
    ...
    process_patch(dataset_name, res_grp, file_idx, label_patch_images, label_patch_metadata, label, seed, rot_num, rotation_mode, sample_patch_size, occupancy_thresh, num_psfs_per_bin, psf_path_list, mean_background_offset, sigma_background_noise, root, ph, l)
