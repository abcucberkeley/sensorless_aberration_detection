...
def data_loader(input_path, gt_path, bsize, pmem, validation_split_pc=0.2):
    ...
    gt_filenames = sorted(os.listdir(gt_path), key=lambda x: (int(x.split('-')[1]), int(x.split('_')[1].split('-')[0]), int(x.split('_')[2].split('.')[0])))
    gt_filenames = np.repeat(gt_filenames, 600)
    ...
    input_filepaths = sorted(os.listdir(input_path), key=lambda x: (int(x.split('-')[1]), int(x.split('_')[1].split('-')[0]), int(x.split('_')[2])))
    ...
    for file in input_filepaths:
        ...
        sorted_input_files = sorted(os.listdir(os.path.join(input_path, file)), key=lambda x: (int(x.split('-')[1]), int(x.split('_')[1].split('-')[0]), int(x.split('_')[2].split('-')[0]), int(x.split('_')[3].split('.')[0]), x.split('_')[3].split('.')[1]))
    ...
    # gt_filenames = sorted(os.listdir(gt_path), key=lambda x: (int(x.split('-', 2)[1]), int(x.split('-', 3)[2].split('_')[-1]), int(x.split('_')[-1].split('.')[0]), x.split('_')[-1].split('.')[-1]))
    # gt_filenames = np.array(gt_filenames)
    # input_filenames = gt_filenames.copy()
